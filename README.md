...
> 当前公开版本为 [`0.2.0`](https://github.com/0dizzy0/remoPhoto/releases/tag/v0.2.0)，APK SHA-256 为 `623a1a889a7952757988e4c65bf49733163a509e77248a351d46fda25da1cf5f`。该版本新增 SMB2/3 只读远程仓库；本地门禁、API 29 覆盖升级、Release SMB Smoke、CI，以及线上附件的 hash、签名、版本和安装启动复验均已通过。
...
- 通过 SMB2/3 只读连接 Windows、Linux Samba 或 NAS 共享，支持中文层级目录、静态图/GIF、离线索引和有界恢复；不支持 SMB1、Guest/匿名或写入远端文件。
- 导入导出数据库与设置，不导出远程凭据。
...
当前 `0.2.0` 已发布，APK SHA-256、验证证据和残留风险见[项目状态](docs/project/项目状态.md)。
...
SMB 开发回归可在 API 29 模拟器、WSL2 Ubuntu/Samba 和专用测试账号已配置后运行：

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\run-smb-smoke.ps1 `
  -BuildDebug `
  -Python "<开发目录中的 python.exe 或 pythonw.exe>"
```

脚本从 `D:\ai_program\remoPhoto-test-data` 复制 30 张测试图片到 WSL 临时 staging，不修改源相册；随后自动完成临时只读共享、安装、添加、同步、静态图/GIF 流、断线恢复、远端增删刷新和仓库删除。`finally` 会恢复 Samba 配置并验证临时目录范围后清理，标准输出最终只有 `PASS` 或 `FAIL`，详细证据写入 `.cache/qa/smb-smoke/`。路径、发行版、端口和凭据文件均可通过脚本参数覆盖。
